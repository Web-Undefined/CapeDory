...
# Created 10/21/95              Last Modified 2/13/04 Walt Bilofsky          #
...
$cgi_url = "http://www.toolworks.com/cgi-bin/wwadmin.cgi";
$basedir = "/home/toolwo5/public_html";
$baseurl = "http://www.toolworks.com";

($board, $command) = split(/=/, $ENV{'QUERY_STRING'});
...
if ($board eq "cd") {
   $cgi_url = "http://www.capedory.org/cgi-bin/wwadmin.cgi";
   $basedir = "/home/toolwo5/public_html/cdsoa";
   $baseurl = "http://www.capedory.org/bboard";
   $dir = "bboard";
   ...
   $dir = "testboard";
   $cgi_url = "http://www.netwiz.com/cgi-bin/twtestboard.cgi";
   ...
   $background = $fbackground = " bgcolor=\"#FFFFFF\"";
...
else { &fatal_error("Unknown Message Board",
   "Internal error: Please <a href=\"mailto:bilofsky\@toolworks.com\">notify the Webmaster</a>.");
   }
...
$basedir  =~ m/^(\S+)$/ || die $!;  $basedir = $1;
$datafile =~ m/^(\S+)$/ || die $!;  $datafile = $1;
$mesgfile =~ m/^(\S+)$/ || die $!;  $mesgfile = $1;
$mesgdir  =~ m/^(\S+)$/ || die $!;  $mesgdir = $1;
$passwd_file  =~ m/^(\S+)$/ || die $!;  $passwd_file = $1;
...
if ($command eq 'remove' || $command eq 'remove_for_sale' ) {
   if ( $command eq 'remove_for_sale' ) { $forstg = "'For Sale' "; }
   print "<html><head><title>Remove $forstg Messages from $title</title></head>\n";
   print "<body><center><h1>Remove $forstg Messages from $title</h1></center>\n";
   ...
   print "which they appear in the $mesgfile page.  This will give you an idea of\n";
   ...
   open(MSGS,"$basedir/$mesgfile") || &error(message_file);
   @lines = <MSGS>;
   close(MSGS);

   foreach $line (@lines) {
	if ($command eq 'remove_for_sale') {
		if ( ! ( $line =~ /sale/i || $line =~ /wanted/i || $line =~ /seek/i 
			 || $line =~ /wtb/i  || $line =~ /looking/i || $line =~ /selling/i ) ) { next; }
		}
      if ($line =~ /<!--(t|top): ?(.*)--><li><a href="$mesgdir\/\2\.$ext">(.*)<\/a> - <b>(.*)<\/b>.*<i>(.*)<\/i>/) {
         push(@ENTRIES,$2);
         $SUBJECT{$2} = $3;
         $AUTHOR{$2} = $4;
         $DATE{$2} = $5;
      }
      elsif ($titles_only != 0 && $line =~ /<!--(t|top): ?(.*)--><li><a href="$mesgdir\/\2\.$ext">(.*)<\/a>/) {
         push(@ENTRIES,$2);
         $SUBJECT{$2} = $3;
         $AUTHOR{$2} = " ";
         $DATE{$2} = " ";
   ...
   print "<table border>\n";
   print "<tr>\n";
   print "<th colspan=6>Username: <input type=text name=\"username\"> -- Password: <input type=password name=\"password\">\n";
   print "<br><input type=submit value=\"Remove Messages\"> <input type=reset> </th>\n";
   print "</tr><tr>\n";
   ...
   foreach (@ENTRIES) {
      print "<tr>\n";
      print "<th><b>$_</b> </th><td>T: <input type=radio name=\"$_\" value=\"all\"> </td><td>S: <input type=radio name=\"$_\" value=\"single\"> </td><td><a href=\"$baseurl/$mesgdir/$_\.$ext\">$SUBJECT{$_} </a></td><td>$AUTHOR{$_} </td><td>$DATE{$_}<br></td>\n";
      print "</tr>\n";
   }
   print "</table>\n";
   print "<p>\n";
...
###########################################################################
   ...
   print "<html><head><title>Remove Messages from $title By Number</title></head>\n";
   print "<body><center><h1>Remove Messages from $title By Number</h1></center>\n";
   print "Select below to remove those postings you wish to remove.\n";
   print "Checking the Input Box on the left will remove the whole thread\n";
   ...
   print "<table border>\n";
   print "<tr>\n";
   print "<th colspan=6>Username: <input type=text name=\"username\"> -- Password: <input type=password name=\"password\"><br>\n";
   print "<input type=submit value=\"Remove Messages\"> <input type=reset> </th>\n";
   print "</tr>\n";
   print "<tr>\n";
   print "<th>Post # </th><th>Thread </th><th>Single </th><th>Subject </th><th> Author</th><th> Date</th></tr>\n";
...
###########################################################################
   ...
   print "<html><head><title>Remove Messages from $title By Date</title></head>\n";
   print "<body><center><h1>Remove Messages from $title By Date</h1></center>\n";
   print "Select below to remove those postings you wish to remove.\n";
...
###########################################################################
   ...
   print "<html><head><title>Remove Messages from $title By Author</title></head>\n";
   print "<body><center><h1>Remove Messages from $title By Author</h1></center>\n";
...
###########################################################################
   ...
   print "<html><head><title>Change $title Admin Password</title></head>\n";
   print "<body><center><h1>Change $title Admin Password</h1></center>\n";
...
print "<html><head><title>Results of Message Board Removal (For Sale)</title></head>\n";
print "<body><center><h1>Results of Message Board Removal (For Sale)</h1></center>\n";
   ...
   foreach $single (@SINGLE) {
      foreach ($j = 0;$j <= @lines;$j++) {
         if ($lines[$j] =~ /<!--(t|top): ?$single-->/) {
            splice(@lines, $j, 3);
            $j -= 3;
         }
         elsif ($lines[$j] =~ /<!--(e|end): ?$single-->/) {
            splice(@lines, $j, 1);
            $j--;
         }
      }
      $filename = "$basedir/$mesgdir/$single\.$ext";
      $filename  =~ m/^(\S+)$/ || die $!;  $filename = $1;   #sanitize for security
   ...
   foreach $all (@ALL) {
      ...
      undef(@DELETE);   # Bug fix - Walt Bilofsky 7/31/98
      foreach ($j = 0;$j <= @lines;$j++) {
         if ($lines[$j] =~ /<!--(t|top): ?$all-->/) {
            $top = $j;
         }
         elsif ($lines[$j] =~ /<!--(e|end): ?$all-->/) {
            $bottom = $j;
         }
      }
      if ($top && $bottom) {
...
###########################################################################
   ...
   print "<html><head><title>WWWAdmin for $title</title></head>\n";   print "<body bgcolor=#FFFFFF text=#000000><center><h1>WWWAdmin for $title</h1></center>\n";
   print "Choose your Method of modifying $title Below:<p>\n";
   print "<hr size=7 width=75%><br>\n";
   print "<ul>\n";
   print "<li>Remove Files\n";
   print "<ul>\n";
   print "<li><a href=\"$cgi_url/?$board=remove\">Remove Files</a>\n";
   print "<li><a href=\"$cgi_url/?$board=remove_by_num\">Remove Files by Number</a>\n";
   print "<li><a href=\"$cgi_url/?$board=remove_by_date\">Remove Files by Date</a>\n";
   print "<li><a href=\"$cgi_url/?$board=remove_for_sale\">Remove 'For Sale'</a>\n";
   print "<li><a href=\"$cgi_url/?$board=remove_by_author\">Remove Files by Author</a>\n";
   print "</ul><br>\n";
   print "<li>Password\n";
   print "<ul>\n";
   print "<li><a href=\"$cgi_url/?$board=change_passwd\">Change Admin Password</a>\n";
   print "</ul>\n";
   print "</ul>\n";
}
...
sub return_html {
   ...
   elsif ($type eq 'remove_for_sale') {
      print "<html><head><title>Results of Message Board Removal (For Sale)</title></head>\n";
      print "<body><center><h1>Results of Message Board Removal (For Sale)</h1></center>\n";
   }
   elsif ($type eq 'change_passwd') {
      print "<html><head><title>$title WWWAdmin Password Changed</title></head>\n";
      print "<body><center><h1>$title WWWAdmin Password Changed</h1></center>\n";
      print "Your Password for $title WWWAdmin has been changed!  Results are below:<p><hr size=7 width=75%><p>\n";
   ...
   if ($type =~ /^remove/) {
      print "Below is a short summary of what messages were removed from $mesgfile and the\n";
      ...
      if (@TOP_BOT) {
         print "<hr><p><b>ERROR: Messages with no matching top and bottom:</b> @TOP_BOT<p><hr><p>\n";
      }
...
sub error {
   ...
   elsif ($error eq 'message_file') {
      print "<html><head><title>Could Not Open Message File For Reading</title></head>\n";
      print "<body><center><h1>Could Not Open Message File For Reading</h1></center>\n";
      print "Could not open the message file for reading!  Check permissions and try again.<p>\n";
      &passwd_trailer
   }
...
sub passwd_trailer {
   print "<hr size=7 width=75%><center><font size=-1>\n";
   print "[ <a href=\"$cgi_url?$board\">WWWAdmin</a> ] [ <a href=\"$baseurl/$mesgfile\">$title</a> ]\n";
   print "</font></center><hr size=7 width=75%>\n";
   print "</body></html>\n";
}
...
sub print_links {
   print "[ <a href=\"$cgi_url/?$board=remove\">Remove</a> ] ";
   print "[ <a href=\"$cgi_url/?$board=remove_by_date\">Remove by Date</a> ] ";
   print "[ <a href=\"$cgi_url/?$board=remove_by_author\">Remove by Author</a> ] ";
   print "[ <a href=\"$cgi_url/?$board=remove_by_num\">Remove by Message Number</a> ] ";
   print "[ <a href=\"$cgi_url/?$board=remove_for_sale\">Remove 'For Sale'</a> ]\n";
   print "[ <a href=\"$baseurl/$mesgfile\">$title</a> ]\n";
   }
